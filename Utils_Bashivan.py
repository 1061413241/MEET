...
def gen_images(locs, features, n_gridpoints, normalize=True,
    ...
    :param features: Feature matrix as [n_samples, n_features]  i.e. [2670,192]
    ...
    assert features.shape[1] % nElectrodes == 0     # 192%64==3
    n_colors = int(features.shape[1] / nElectrodes)
    for c in range(n_colors):
        feat_array_temp.append(features[:, c * nElectrodes : nElectrodes * (c+1)])      # [2670,64]
    if augment:
        if pca:
            for c in range(n_colors):
    ...
    n_samples = features.shape[0]   # 2670
