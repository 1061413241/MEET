...
from meet.models.vit_utils import DropPath, to_2tuple, trunc_normal_
...
class VisionTransformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=5, num_classes=1000, embed_dim=768, depth=12,
...
class MEET(nn.Module):
    def __init__(self, img_size=32, patch_size=16, embed_dim=768, 
                 depth=12, num_heads=12, num_classes=4, num_frames=6, 
...
def meet_small_patch8(num_classes: int = 3):
    """
    MEET-Small model from original paper ().
    """
    model = MEET(img_size=32, 
                 patch_size=8,
                 embed_dim=768,
                 depth=3,
                 num_heads=3,
...
def meet_base_patch16(num_classes: int = 3):
    """
    MEET-Base model from original paper ().
    """
    model = MEET(img_size=32, 
                 patch_size=16,
                 embed_dim=768,
...
def meet_large_patch16(num_classes: int = 3):
    """
    MEET-Large model from original paper ().
    """
    model = MEET(img_size=32, 
                 patch_size=16,
                 embed_dim=1024,
